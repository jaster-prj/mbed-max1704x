...
MAX1704X::MAX1704X(PinName sda, PinName scl, uint8_t address) {
  _i2c = new I2C(sda, scl);
  _address = address;
}

MAX1704X::MAX1704X(I2C *i2c) {
  _i2c = i2c;
}

MAX1704X::MAX1704X(I2C *i2c, uint8_t address) {
  _i2c = i2c;
  _address = address;
}

// Change the device type if required. Do this after instantiation but before .begin
void MAX1704X::setDevice(max1704x_devices_e device)
{
...
bool MAX1704X::isConnected(void)
{
  ...
  {
    int err = _i2c->write(_address<<1, &reg, 1);

    if (!err) {
      err = _i2c->read(_address<<1, (char *)ret_val, 2);
      version = ((uint16_t)ret_val[0] << 8) | ret_val[1];
      success = true;
    } else {
      retries--;
      ...
      if (_printDebug == true)
      {
        if(_debugPort != nullptr) {
          _debugPort->write("MAX1704X::isConnected: retrying...", 35);
        } else {
          printf("MAX1704X::isConnected: retrying...");
        }
      }
  ...
  if (!success) // Return now if the version could not be read
  {
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("MAX1704X::isConnected: failed to detect IC!", 44);
      } else {
        printf("MAX1704X::isConnected: failed to detect IC!");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return (success);
...
void MAX1704X::enableDebugging(BufferedSerial &debugPort)
{
  #if MAX1704X_ENABLE_DEBUGLOG
...
void MAX1704X::disableDebugging(void)
{
  #if MAX1704X_ENABLE_DEBUGLOG
...
uint8_t MAX1704X::getID()
{
  if (_device <= MAX1704X_MAX17044)
  {
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("getID: not supported on this device", 36);
      } else {
        printf("getID: not supported on this device");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return (0);
...
uint8_t MAX1704X::setResetVoltage(uint8_t threshold)
{
  if (_device <= MAX1704X_MAX17044)
  {
    #if MAX1704X_ENABLE_DEBUGLOG
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("setResetVoltage: not supported on this device", 46);
      } else {
        printf("setResetVoltage: not supported on this device");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
...
uint8_t MAX1704X::setResetVoltage(float threshold)
{
  // 7 bits. LSb = 40mV
  uint8_t thresh = (uint8_t)(constrain(threshold, (float)0.0, (float)5.08) / 0.04);
  return setResetVoltage(thresh);
}

uint8_t MAX1704X::getResetVoltage(void)
{
  if (_device <= MAX1704X_MAX17044)
  {
    #if MAX1704X_ENABLE_DEBUGLOG
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("getResetVoltage: not supported on this device", 46);
      } else {
        printf("getResetVoltage: not supported on this device");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return (0);
...
uint8_t MAX1704X::enableComparator(void)
{
  if (_device <= MAX1704X_MAX17044)
  {
    #if MAX1704X_ENABLE_DEBUGLOG
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("enableComparator: not supported on this device", 47);
      } else {
        printf("enableComparator: not supported on this device");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return (MAX17043_GENERIC_ERROR);
...
uint8_t MAX1704X::disableComparator(void)
{
  if (_device <= MAX1704X_MAX17044)
  {
    #if MAX1704X_ENABLE_DEBUGLOG
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("disableComparator: not supported on this device", 48);
      } else {
        printf("disableComparator: not supported on this device");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return (MAX17043_GENERIC_ERROR);
...
float MAX1704X::getChangeRate(void)
{
  if (_device <= MAX1704X_MAX17044)
  {
    #if MAX1704X_ENABLE_DEBUGLOG
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("getChangeRate: not supported on this device", 44);
      } else {
        printf("getChangeRate: not supported on this device");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return (0.0);
...
uint8_t MAX1704X::getStatus(void)
{
  if (_device <= MAX1704X_MAX17044)
  {
    #if MAX1704X_ENABLE_DEBUGLOG
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("getStatus: not supported on this device", 40);
      } else {
        printf("getStatus: not supported on this device");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return (0);
...
bool MAX1704X::isReset(bool clear)
{
...
bool MAX1704X::isVoltageHigh(bool clear)
{
  uint8_t status = getStatus();
...
bool MAX1704X::isVoltageLow(bool clear)
{
  uint8_t status = getStatus();
...
bool MAX1704X::isVoltageReset(bool clear)
{
  uint8_t status = getStatus();
...
bool MAX1704X::isLow(bool clear)
{
  uint8_t status = getStatus();
...
bool MAX1704X::isChange(bool clear)
{
  uint8_t status = getStatus();
...
uint8_t MAX1704X::clearStatusRegBits(uint16_t mask)
{
...
uint8_t MAX1704X::getAlert(bool clear)
{
  // Read config reg, so we don't modify any other values:
  uint16_t configReg = read16(MAX17043_CONFIG);
...
bool MAX1704X::enableSOCAlert()
{
  if (_device <= MAX1704X_MAX17044)
  {
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("enableSOCAlert: not supported on this device", 45);
      } else {
        printf("enableSOCAlert: not supported on this device");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return (false);
...
bool MAX1704X::disableSOCAlert()
{
  if (_device <= MAX1704X_MAX17044)
  {
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("disableSOCAlert: not supported on this device", 46);
      } else {
        printf("disableSOCAlert: not supported on this device");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return (false);
...
uint8_t MAX1704X::enableAlert(void)
{
  if (_device <= MAX1704X_MAX17044)
  {
    #if MAX1704X_ENABLE_DEBUGLOG
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("enableAlert: not supported on this device", 42);
      } else {
        printf("enableAlert: not supported on this device");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return (MAX17043_GENERIC_ERROR);
...
uint8_t MAX1704X::disableAlert(void)
{
  if (_device <= MAX1704X_MAX17044)
  {
    #if MAX1704X_ENABLE_DEBUGLOG
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("disableAlert: not supported on this device", 43);
      } else {
        printf("disableAlert: not supported on this device");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return (MAX17043_GENERIC_ERROR);
...
uint8_t MAX1704X::setThreshold(uint8_t percent)
{
  ...
  // The value is (32 - ATHD)%, e.g.: 00000=32%, 00001=31%, 11111=1%.
  // Let's convert our percent to that first:
  percent = (uint8_t)constrain((float)percent, (float)0.0, (float)32.0);
  percent = 32 - percent;
...
uint8_t MAX1704X::sleep()
{
  ...
  if (configReg & MAX17043_CONFIG_SLEEP)
  {
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("sleep: MAX17043 is already sleeping!", 37);
      } else {
        printf("sleep: MAX17043 is already sleeping!");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
...
uint8_t MAX1704X::wake()
{
  ...
  if (!(configReg & MAX17043_CONFIG_SLEEP))
  {
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("wake: MAX17043 is already awake!", 33);
      } else {
        printf("wake: MAX17043 is already awake!");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return MAX17043_GENERIC_ERROR; // Already sleeping, do nothing but return an error
...
uint8_t MAX1704X::reset()
{
  return write16(MAX17043_COMMAND_POR, MAX17043_COMMAND);
}

uint8_t MAX1704X::getCompensation()
{
  uint16_t configReg = read16(MAX17043_CONFIG);
...
uint16_t MAX1704X::getConfigRegister()
{
  return read16(MAX17043_CONFIG);
}

uint8_t MAX1704X::setCompensation(uint8_t newCompensation)
{
...
uint8_t MAX1704X::setVALRTMax(uint8_t threshold)
{
  if (_device <= MAX1704X_MAX17044)
  {
    #if MAX1704X_ENABLE_DEBUGLOG
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("setVALRTMax: not supported on this device", 42);
      } else {
        printf("setVALRTMax: not supported on this device");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return (MAX17043_GENERIC_ERROR);
...
uint8_t MAX1704X::setVALRTMax(float threshold)
{
  uint8_t thresh = (uint8_t)(constrain(threshold, (float)0.0, (float)5.1) / 0.02);
  return setVALRTMax(thresh);
}

uint8_t MAX1704X::getVALRTMax()
{
  if (_device <= MAX1704X_MAX17044)
  {
    #if MAX1704X_ENABLE_DEBUGLOG
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("getVALRTMax: not supported on this device", 42);
      } else {
        printf("getVALRTMax: not supported on this device");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return (0);
...
uint8_t MAX1704X::setVALRTMin(uint8_t threshold)
{
  if (_device <= MAX1704X_MAX17044)
  {
    #if MAX1704X_ENABLE_DEBUGLOG
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("setVALRTMin: not supported on this device", 42);
      } else {
        printf("setVALRTMin: not supported on this device");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return (MAX17043_GENERIC_ERROR);
...
uint8_t MAX1704X::setVALRTMin(float threshold)
{
  uint8_t thresh = (uint8_t)(constrain(threshold, (float)0.0, (float)5.1) / 0.02);
  return setVALRTMin(thresh);
}

uint8_t MAX1704X::getVALRTMin()
{
  if (_device <= MAX1704X_MAX17044)
  {
    #if MAX1704X_ENABLE_DEBUGLOG
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("getVALRTMin: not supported on this device", 42);
      } else {
        printf("getVALRTMin: not supported on this device");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return (0);
...
bool MAX1704X::isHibernating()
{
  if (_device <= MAX1704X_MAX17044)
  {
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("isHibernating: not supported on this device", 44);
      } else {
        printf("isHibernating: not supported on this device");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return (false);
...
uint8_t MAX1704X::getHIBRTActThr()
{
  if (_device <= MAX1704X_MAX17044)
  {
    #if MAX1704X_ENABLE_DEBUGLOG
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("getHIBRTActThr: not supported on this device", 45);
      } else {
        printf("getHIBRTActThr: not supported on this device");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return (0);
...
uint8_t MAX1704X::setHIBRTActThr(uint8_t threshold)
{
  if (_device <= MAX1704X_MAX17044)
  {
    #if MAX1704X_ENABLE_DEBUGLOG
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("setHIBRTActThr: not supported on this device", 45);
      } else {
        printf("setHIBRTActThr: not supported on this device");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return (MAX17043_GENERIC_ERROR);
...
uint8_t MAX1704X::setHIBRTActThr(float threshold)
{
  // LSb = 1.25mV
  uint8_t thresh = (uint8_t)(constrain(threshold, (float)0.0, (float)0.31875) / 0.00125);
  return setHIBRTActThr(thresh);
}

uint8_t MAX1704X::getHIBRTHibThr()
{
  if (_device <= MAX1704X_MAX17044)
  {
    #if MAX1704X_ENABLE_DEBUGLOG
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("getHIBRTHibThr: not supported on this device", 45);
      } else {
        printf("getHIBRTHibThr: not supported on this device");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return (0);
...
uint8_t MAX1704X::setHIBRTHibThr(uint8_t threshold)
{
  if (_device <= MAX1704X_MAX17044)
  {
    #if MAX1704X_ENABLE_DEBUGLOG
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("setHIBRTHibThr: not supported on this device", 45);
      } else {
        printf("setHIBRTHibThr: not supported on this device");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return (MAX17043_GENERIC_ERROR);
...
uint8_t MAX1704X::setHIBRTHibThr(float threshold)
{
  // LSb = 0.208%/hr
  uint8_t thresh = (uint8_t)(constrain(threshold, (float)0.0, (float)53.04) / 0.208);
  return setHIBRTHibThr(thresh);
}

uint8_t MAX1704X::enableHibernate()
{
  if (_device <= MAX1704X_MAX17044)
  {
    #if MAX1704X_ENABLE_DEBUGLOG
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("enableHibernate: not supported on this device", 46);
      } else {
        printf("enableHibernate: not supported on this device");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return (MAX17043_GENERIC_ERROR);
...
uint8_t MAX1704X::disableHibernate()
{
  if (_device <= MAX1704X_MAX17044)
  {
    #if MAX1704X_ENABLE_DEBUGLOG
    ...
    {
      if(_debugPort != nullptr) {
        _debugPort->write("disableHibernate: not supported on this device", 47);
      } else {
        printf("disableHibernate: not supported on this device");
      }
    }
    #endif // if MAX1704X_ENABLE_DEBUGLOG
    return (MAX17043_GENERIC_ERROR);
...
uint8_t MAX1704X::write16(uint16_t data, uint8_t address)
{
  uint8_t buffer[3] = {address,(uint8_t)((data & 0xFF00) >> 8),(uint8_t)(data & 0x00FF)};

  return (uint8_t)(_i2c->write(_address<<1,(const char*) buffer, 3));
}

uint16_t MAX1704X::read16(uint8_t address)
{
  bool success = false;
  uint8_t retries = 3;
  uint16_t result = 0;
  ...
  while ((success == false) && (retries > 0))
  {
    err = _i2c->write(_address<<1, (const char*) &address, 1);
    if(!err)
    {
      err = _i2c->read(_address<<1,(char *) data, 2);
    }
    if(!err) {
      ...
      {
        if(_debugPort != nullptr) {
          _debugPort->write("MAX1704X::read16: retrying...", 30);
        } else {
          printf("MAX1704X::read16: retrying...");
        }
      }
      #endif // if MAX1704X_ENABLE_DEBUGLOG
      ThisThread::sleep_for(50ms);
  ...
  if (_printDebug == true)
  {
    if (!success) {
      if(_debugPort != nullptr) {
        _debugPort->write("MAX1704X::read16: failed to read data!", 39);
      } else {
        printf("MAX1704X::read16: failed to read data!");
      }
    }
  }
  #endif // if MAX1704X_ENABLE_DEBUGLOG
