/******************************************************************************
Fork of https://github.com/sparkfun/SparkFun_MAX1704x_Fuel_Gauge_Arduino_Library
...
{
public:
  MAX1704X(PinName sda, PinName scl);
  MAX1704X(PinName sda, PinName scl, uint8_t address);
  MAX1704X(I2C *i2c);
  MAX1704X(I2C *i2c, uint8_t address);

  // Change the device type if required. Do this after instantiation but before .begin
  void setDevice(max1704x_devices_e device);

  //Returns true if device is present
  bool isConnected(void);

  // Debug
  void enableDebugging(BufferedSerial &debugPort); // enable debug messages
...
private:
  ...
  BufferedSerial *_debugPort = nullptr;          //The stream to send debug messages to if enabled. Usually Serial.
  ...
  uint8_t _address = MAX1704x_ADDRESS;
